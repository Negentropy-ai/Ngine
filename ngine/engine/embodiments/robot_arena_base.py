...
from ngine.utils.isaaclab_utils import get_robot_joint_target_from_scene
from ngine.utils.log_utils import get_action_space_def, get_camera_metadata
from ngine.utils.place_utils.env_utils import get_safe_robot_anchor
...
class EmbodimentGeneralObsCfg(ObsGroup):
    actions: ObsTerm = ObsTerm(func=mdp.last_action)
    joint_pos: ObsTerm = ObsTerm(func=mdp.joint_pos)
    joint_vel: ObsTerm = ObsTerm(func=mdp.joint_vel)
    joint_pos_rel: ObsTerm = ObsTerm(func=mdp.joint_pos_rel)
    joint_vel_rel: ObsTerm = ObsTerm(func=mdp.joint_vel_rel)
    eef_pos: ObsTerm = ObsTerm(func=mdp.ee_frame_pos)
    eef_quat: ObsTerm = ObsTerm(func=mdp.ee_frame_quat)
...
class RobotBase(EmbodimentBase):
